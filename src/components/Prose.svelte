<script>
    import content from "$data/content.csv"
    // Keep only content that has category "Poetry"
    const prose = content.filter(item => item.category === "prose");
    import star from "$svg/star.svg"
  </script>
  
<!-- Section 1 -->
<section id="prose" class="flex items-center max-w-9xl md:px-0 mx-6 md:mx-auto   justify-center   ">
    <div class="py-12">
        <div class="container py-12 border-2 border-[color:var(--color-background)] flex flex-col items-start mx-auto lg:items-center">
            <h2 class="relative flex items-start justify-start w-full max-w-3xl text-[6rem] lg:text-[6rem] font-cormorant  text-[color:var(--color-background)] font-bold lg:justify-center">
                <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="absolute right-0 hidden w-12 h-12 -mt-2 -mr-16 text-[color:var(--color-background)] lg:inline-block" viewBox="0 0 975.036 975.036">
                <path d="M925.036 57.197h-304c-27.6 0-50 22.4-50 50v304c0 27.601 22.4 50 50 50h145.5c-1.9 79.601-20.4 143.3-55.4 191.2-27.6 37.8-69.399 69.1-125.3 93.8-25.7 11.3-36.8 41.7-24.8 67.101l36 76c11.6 24.399 40.3 35.1 65.1 24.399 66.2-28.6 122.101-64.8 167.7-108.8 55.601-53.7 93.7-114.3 114.3-181.9 20.601-67.6 30.9-159.8 30.9-276.8v-239c0-27.599-22.401-50-50-50zM106.036 913.497c65.4-28.5 121-64.699 166.9-108.6 56.1-53.7 94.4-114.1 115-181.2 20.6-67.1 30.899-159.6 30.899-277.5v-239c0-27.6-22.399-50-50-50h-304c-27.6 0-50 22.4-50 50v304c0 27.601 22.4 50 50 50h145.5c-1.9 79.601-20.4 143.3-55.4 191.2-27.6 37.8-69.4 69.1-125.3 93.8-25.7 11.3-36.8 41.7-24.8 67.101l35.9 75.8c11.601 24.399 40.501 35.2 65.301 24.399z">
                </path>
            </svg>
               Prose</h2>

            <div class="items-center text-[color:var(--color-background)] px-12  justify-center w-full mt-12 mb-4 lg:flex">
                <div class="flex flex-col hover:bg-[color:var(--color-text)]  border-b-2 lg:border-none border-gray-300 hover:cursor-pointer hover:text-[color:var(--color-background)] items-start justify-start w-full h-auto px-0 mx-0 mb-12  border-transparent lg:w-1/3 lg:mb-0 lg:px-8 lg:border-[color:var(--color-background)]">
                    <a href="/prose/{prose[0].slug}">

                    <div class="flex flex-col items-start lg:items-center h-[100px] lg:h-[300px] justify-evenly">
                        {#if prose[0].featured == "TRUE"}
                        <div class="flex py-1 justify-center items-center">{@html star} <span class="text-[color:#eac638] px-2 font-bold uppercase"> Curated</span></div>
                        {/if}
                        <h3 class="font-bold text-left lg:text-center font-dm  text-2xl lg:text-4xl capitalize ">{prose[0].title}</h3>
                        <h4 class="font-bold text-left font-mono  lg:text-xl text-md capitalize  ">{prose[0].author}</h4>
                    </div>
                    </a>
                   
                </div>
                    <div class="flex flex-col hover:bg-[color:var(--color-text)] border-b-2 lg:border-b-[0] border-gray-300 hover:cursor-pointer hover:text-[color:var(--color-background)] items-start lg:border-l lg:border-r justify-start w-full h-auto px-0 mx-0 mb-12  border-transparent lg:w-1/3 lg:mb-0 lg:px-8 lg:border-[color:var(--color-background)]">
                        <a href="/prose/{prose[1].slug}">

                        <div class="flex flex-col items-start lg:items-center h-[100px] lg:h-[300px] justify-evenly">
                            {#if prose[1].featured == "TRUE"}
                            <div class="flex py-1 justify-center items-center">{@html star} <span class="text-[color:#eac638] px-2 font-bold uppercase"> Curated</span></div>
                            {/if}
                            <h3 class="font-bold text-left lg:text-center font-dm  text-2xl lg:text-4xl capitalize ">{prose[1].title}</h3>
                            <h4 class="font-bold text-left font-mono  lg:text-xl text-md capitalize  ">{prose[1].author}</h4>
                        </div>          
                        
                        </a>
                </div>

                <div class="flex flex-col hover:bg-[color:var(--color-text)] border-b-2 lg:border-b-[0] border-gray-300 hover:cursor-pointer hover:text-[color:var(--color-background)] items-start  lg:border-r justify-start w-full h-auto px-0 mx-0 mb-12  border-transparent lg:w-1/3 lg:mb-0 lg:px-8 lg:border-[color:var(--color-background)]">
                    <a href="/prose/{prose[2].slug}">

                    <div class="flex flex-col items-start lg:items-center h-[100px] lg:h-[300px] justify-evenly">
                        {#if prose[2].featured == "TRUE"}
                        <div class="flex py-1 justify-center items-center">{@html star} <span class="text-[color:#eac638] px-2 font-bold uppercase"> Curated</span></div>
                        {/if}
                        <h3 class="font-bold text-left lg:text-center font-dm  text-2xl lg:text-4xl capitalize ">{prose[2].title}</h3>
                        <h4 class="font-bold text-left font-mono  lg:text-xl text-md capitalize  ">{prose[2].author}</h4>
                    </div>
                   </a> 
                </div>

                <div class="flex flex-col hover:bg-[color:var(--color-text)] border-b-2  lg:border-none border-gray-300 hover:cursor-pointer hover:text-[color:var(--color-background)] items-start justify-start w-full h-auto px-0 mx-0 mb-12  border-transparent lg:w-1/3 lg:mb-0 lg:px-8 lg:border-[color:var(--color-background)]">
                    <a class="w-full" href="/prose/{prose[3].slug}">

                    <div class="flex flex-col  lg:items-center h-[100px] lg:h-[300px] justify-evenly">
                        {#if prose[3].featured == "TRUE"}
                        <div class="flex py-1 justify-center items-center">{@html star} <span class="text-[color:#eac638] px-2 font-bold uppercase"> Curated</span></div>
                        {/if}
                        <h3 class="font-bold text-left lg:text-center font-dm  text-2xl lg:text-4xl capitalize ">{prose[3].title}</h3>
                        <h4 class="font-bold text-left font-mono  lg:text-xl text-md capitalize  ">{prose[3].author}</h4>
                    </div>
                   </a> 
                </div>
            </div>
        </div>
    </div>
</section>

<style>
    #prose {
        background-image: url("/assets/home4.jpg");
        background-size: cover;
        background-position: 0 -190px;
    }
</style>
...
<section class="relative bg-[color:#e3d5ca] w-full">
    <div class="relative w-full  py-10 mx-auto sm:py-12 md:py-16 md:px-16  max-w-9xl">
        ...
        <div class=" grid h-full grid-cols-1 px-8 lg:grid-cols-12 gap-10 pb-10 mt-8 ">
        {#each photoEssay as item, i}
                ...
                <a href="{base}/photo-essay/{item.slug}" class="block w-full transition duration-300 ease-in-out transform bg-center bg-cover h-[420px] hover:scale-110" style="background-image:url('{base}/images/{item.image}'); filter:sepia(0.5);">
</a>
                <div class="relative z-20 w-full h-[180px] py-8 text-[color:var(--color-text)] bg-[color:#d6c2b3] border-t-0 border-yellow-200 px-7">
                    <a href="{base}/photo-essay/{item.slug}" class="inline-block text-xs font-semibold absolute top-0 -mt-3.5 px-4 py-2 uppercase bg-[color:#B99379] text-white">{item.author}</a>
                    <h2 class="text-3xl group-hover:underline font-bold"><a href="{base}/photo-essay/{item.slug}">{item.title}</a></h2>
                </div>
            </div>
         {/each}
        </div>
